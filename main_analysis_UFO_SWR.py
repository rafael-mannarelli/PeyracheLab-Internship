...
def compute_point_process_cross_correlation(events1, events2, bin_size):
    total_duration = max(max(events1), max(events2))
    t_bins = np.arange(0, total_duration + bin_size, bin_size)
    binned1 = np.histogram(events1, bins=t_bins)[0]
    binned2 = np.histogram(events2, bins=t_bins)[0]

    # Centering the signals by subtracting the mean
    binned1_centered = (binned1 - np.mean(binned1))#/np.std(binned1)
    binned2_centered = (binned2 - np.mean(binned2))#/np.std(binned2)

    # Computing the cross-correlation
    corr = signal.correlate(binned1_centered, binned2_centered, mode='full',method='fft')
    corr /= np.max(corr)

    # Computing lags
    lags = signal.correlation_lags(len(binned1_centered), len(binned2_centered))*bin_size
    #lags = bin_size * (np.arange(len(corr)) - len(binned1_centered) + 1)

    return lags, corr

def plot_cross_correlation(lags, corr,max_lags,save_fig_path):
    plt.figure(figsize=(10, 5))
    plt.plot(lags, corr)
    plt.title('Cross-Correlation between SWR and UFO Events')
    plt.xlabel('Lag (seconds)')
    plt.ylabel('Cross-correlation')
    plt.xlim([-max_lags,max_lags])
    plt.grid(True)
    plt.savefig(save_fig_path+r'\cc_swr_ufo.svg')
    plt.savefig(save_fig_path+r'\cc_swr_ufo.png', dpi=300,bbox_inches='tight')
    plt.close()

def compute_jpeth(times1, times2, window, bin_size):
    """
    Compute the Joint Peri-event Time Histogram (JPETH).

    Parameters:
    - times1 (np.array): Timestamps of events of type 1.
    - times2 (np.array): Timestamps of events of type 2.
    - window (tuple): Time window (pre-event time, post-event time) in seconds.
    - bin_size (float): Bin size for the histogram in seconds.

    Returns:
    - jpeth_matrix (np.array): 2D histogram matrix of joint event times.
    - bins (np.array): Array of bin edges.
    """
    pre_time, post_time = window
    bins = np.arange(pre_time, post_time + bin_size, bin_size)
    jpeth_matrix = np.zeros((len(bins) - 1, len(bins) - 1))

    for t1 in times1:
        relative_times = times2 - t1
        # Filter times to fall within the specified window
        valid_times = relative_times[(relative_times >= pre_time) & (relative_times <= post_time)]
        # Histogram of valid times
        hist, _ = np.histogram(valid_times, bins=bins)
        # Outer product of the histogram with itself
        jpeth_matrix += np.outer(hist, hist)
    
    return jpeth_matrix, bins[:-1]

def plot_jpeth(jpeth_matrix, bins,save_fig_path):
    """
    Plot the JPETH.

    Parameters:
    - jpeth_matrix (np.array): The JPETH matrix to plot.
    - bins (np.array): The bin edges used in the JPETH matrix.
    """
    plt.figure(figsize=(8, 6))
    plt.imshow(jpeth_matrix, extent=(bins[0], bins[-1], bins[0], bins[-1]), origin='lower', interpolation='none', aspect='auto',cmap='turbo')
    plt.colorbar(label='Counts')
    plt.xlabel('Time from Event 1 (s)')
    plt.ylabel('Time from Event 2 (s)')
    plt.title('Joint Peri-event Time Histogram (JPETH)')
    plt.savefig(save_fig_path+r'\jpeth.svg')
    plt.savefig(save_fig_path+r'\jpeth.png', dpi=300,bbox_inches='tight')
    plt.close()


def plot_jpeth_diagonal(jpeth_matrix, bins, save_fig_path):
    """
    Plot the diagonal of the JPETH matrix to show synchrony over time.

    Parameters:
    - jpeth_matrix (np.array): The JPETH matrix computed earlier.
    - bins (np.array): The bins used to compute the JPETH matrix, representing time from each event.
    """
    # Extract the diagonal of the JPETH matrix
    diagonal = np.diag(jpeth_matrix)
    time_lags = bins  # Time lags corresponding to the diagonal

    # Plotting the diagonal
    plt.figure(figsize=(10, 6))
    plt.plot(time_lags, diagonal)
    plt.title('Diagonal of JPETH (Synchrony over Time)')
    plt.xlabel('Time from Event 1 to Event 2 (s)')
    plt.ylabel('Counts')
    plt.grid(True)
    plt.savefig(save_fig_path+r'\jpeth_diag.svg')
    plt.savefig(save_fig_path+r'\jpeth_diag.png', dpi=300,bbox_inches='tight')
    plt.close()

    
def compute_coincidences(events1, events2, coincidence_window):
    """
    Compute coincidences between two event arrays within a specified window.

    Parameters:
    - events1 (np.array): Timestamps of the first event type.
    - events2 (np.array): Timestamps of the second event type.
    - coincidence_window (float): The time window (in the same units as timestamps) within which
                                  events are considered coincident.

    Returns:
    - coincidence_counts (np.array): Array of coincidence counts per event in events1.
    """
    coincidence_counts = []
    for event_time in events1:
        # Count events in events2 that are within the window of the current event in events1
        count = np.sum((events2 >= event_time - coincidence_window) & 
                       (events2 <= event_time + coincidence_window))
        coincidence_counts.append(count)
    return np.array(coincidence_counts)

def plot_coincidence_histogram(coincidence_counts, save_fig_path):
    """
    Plot a histogram of coincidence counts.

    Parameters:
    - coincidence_counts (np.array): Array of coincidence counts per event.
    - title (str): Title of the histogram plot.
    """
    plt.figure(figsize=(10, 6))
    plt.hist(coincidence_counts, bins=np.arange(np.max(coincidence_counts)+2)-0.5, alpha=0.7, color='blue')
    plt.xlabel('Number of Coincidences')
    plt.ylabel('Frequency')
    plt.title('Coincidence Histogram')
    plt.grid(True)
    plt.xticks(np.arange(np.max(coincidence_counts)+1))
    plt.savefig(save_fig_path+r'\coincidence_hist.svg')
    plt.savefig(save_fig_path+r'\coincidence_hist.png', dpi=300,bbox_inches='tight')
    plt.close()
...
###############################################################################################
    ...
    print("Number of UFOs:", len(ufo_ep))
    print("Number of SWRs:", len(rip_ep))
    print("UFOs rate:", nap.TsGroup({0:ufo_ts}).restrict(sws_ep))
    print("SWRs rate:", nap.TsGroup({0:rip_ts}).restrict(sws_ep))

    if 'SWR/UFO' in compute:
        events_rip=rip_ts.restrict(sws_ep).t
        events_ufo=ufo_ts.restrict(sws_ep).t

        max_lags=100
        lags_pp, corr_pp = compute_point_process_cross_correlation(events_rip, events_ufo, bin_size=0.1)
        plot_cross_correlation(lags_pp, corr_pp,max_lags,save_fig_path) 

        window = (-0.1, 0.1)  # Time window in seconds
        bin_size = 0.005  # Bin size in seconds

        # Compute and plot JPETH
        jpeth_matrix, bins = compute_jpeth(events_rip, events_ufo, window, bin_size)
        plot_jpeth(jpeth_matrix, bins, save_fig_path)
        plot_jpeth_diagonal(jpeth_matrix, bins,save_fig_path)

        coincidence_window = 10  # Example coincidence window (same units as timestamps)
        # Compute coincidences
        #coincidence_counts = compute_coincidences(events_rip, events_ufo, coincidence_window)

        # Plot coincidence histogram
        #plot_coincidence_histogram(coincidence_counts,save_fig_path)
